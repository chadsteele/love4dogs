...
async function createSession() {
	...
	if (!identifier || !secret) {
		logAuthDebug('createSession:missing_credentials', identifier, secret, null, new Error('Missing Bluesky credentials'));
	...
	try {
		...
		logAuthDebug('createSession:network_error', identifier, secret, null, err);
		throw err;
	}
	...
	if (!res.ok) {
		...
		logAuthDebug('createSession:failed_response', identifier, secret, res, new Error(`Session creation failed: ${bodyText}`));
...
export async function POST({ request }) {
	try {
		...
		if (isChat) {
			...
			if (htmlRegex.test(normalizedText)) {
				return new Response(JSON.stringify({ error: 'HTML tags are not allowed in comments.' }), {
					status: 400,
					headers: { 'content-type': 'application/json' }
				});
			}
			...
			if (hasExternalUrl) {
				return new Response(JSON.stringify({ error: 'External links or URLs are not allowed in comments.' }), {
					status: 400,
					headers: { 'content-type': 'application/json' }
				});
			}

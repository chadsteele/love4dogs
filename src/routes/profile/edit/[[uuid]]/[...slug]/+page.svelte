<script>
	...
	import {isIdentityPostType, isKnownPostType, postTypes} from "$lib/config"
	import {
	...
	let editingRouteUuid = $state("")
	let existingProfilePostUris = $state([])
	...
	function extractProfilePostUris(bundle = null) {
	...
	async function deleteProfilePostsByUris(uris = []) {
		const targets = [
			...new Set(
		...
		const response = await fetch("/api/post", {
			...
			body: JSON.stringify({uris: targets}),
		})
		if (!response.ok) {
			const body = await response.json().catch(() => ({}))
			throw new Error(
				body?.error ||
					"New profile was published, but deleting the old profile posts failed.",
			)
		}
	}
	...
	const typeError = $derived(
		isKnownPostType(profileType) ? "" : "Post type is required.",
	)
	const typeAllowsProfileImages = $derived.by(() => {
		return isIdentityPostType(profileType)
	})
	...
	const profileImageError = $derived(
		typeAllowsProfileImages && !selectedProfileImage
	...
	const primaryPostPayload = $derived({
		...
		profilePic: typeAllowsProfileImages
			? uploadedProfileImage?.bskyUrl || null
			: null,
		backgroundPic: typeAllowsProfileImages
			? uploadedBackgroundImage?.bskyUrl || null
			: null,
		name: profileName,
	...
	function applyBundleToEditor(bundle = null, fallbackUuid = "") {
		...
		profileType = String(primary?.type || "")
		...
		editorMediaList = []
		existingProfilePostUris = extractProfilePostUris(bundle)
	}
	...
	function validateRequiredFields() {
		...
		const submitProfileImageError = !typeAllowsProfileImages
			? ""
			: !hasProfileImage
				? "Profile picture is required."
		...
		debugProfile("[profile] validateRequiredFields", {
			...
			hasType: Boolean(profileType),
			hasEmail: Boolean(email.trim()),
			typeAllowsProfileImages,
	...
	async function publishToBluesky() {
		...
		try {
			...
			const publishedSlugPath = cleanCanonicalName(profileName)
				...
				.join("/")
			const publishedViewUrl = `/post/view/${encodeURIComponent(uuid)}/${publishedSlugPath || "post"}`
			...
			if (
				...
				const selectedProfileUrl = String(
					uploadedProfileImage?.bskyUrl ||
						selectedProfileImage?.bskyUrl ||
						selectedProfileImage?.url ||
						"",
				).trim()
			...
			const currentPrimaryUri = String(
				publishResult?.primaryResult?.uri || "",
			)
			const shouldReplaceOldProfile = Boolean(editingRouteUuid)
			const profileUrisToDelete = shouldReplaceOldProfile
				? existingProfilePostUris.filter(
						(uri) => uri && uri !== currentPrimaryUri,
					)
				: []
			if (profileUrisToDelete.length > 0) {
				await deleteProfilePostsByUris(profileUrisToDelete)
				existingProfilePostUris = profileUrisToDelete.filter(
					(uri) => uri === currentPrimaryUri,
				)
			}
	...
	onMount(() => {
		...
		if (routeUuid && typeof localStorage !== "undefined") {
			...
			if (viewCacheData) {
				...
				// Resolve current server-side post URIs for safe replacement deletes after publish.
				fetch(
...
<main class="page">
	...
	<section class="panel">
		<fieldset class="type-fieldset">
			<legend> Is this a post or a profile? </legend>
			<div
				class="radio-inputs"
				class:invalid-field={(validationActive || publishAttempted) &&
					!!typeError}
			>
		...
		{#if publishAttempted && typeError}
			<p class="field-error type-warning">{typeError}</p>
		{/if}
		{#if typeAllowsProfileImages}
			<div
		...
		{/if}
		{#if validationActive && typeAllowsProfileImages && profileImageError}
		...
		{#if validationActive && !publishAttempted && typeError}
			<p class="field-error">{typeError}</p>
		{/if}
		...
		<div class="upload-progress-bar-wrap">
			...
			></div>
			<p class="upload-progress-label">{mediaUploadLabel}</p>
		</div>
		{#if typeAllowsProfileImages}
			<label>
				<span>Private</span>
...
<style>
	...
	.radio-inputs.invalid-field {
		box-shadow: 0 0 0 2px rgba(181, 58, 43, 0.3);
	}
	.radio-inputs .radio {
		flex: 0 0 auto;
		text-align: center;
	...
	.radio-inputs .radio .name {
		...
		color: rgba(81, 101, 90, 1);
		transition: all 0.15s ease-in-out;
		background: linear-gradient(180deg, #fffdf9 0%, #efe6db 100%);
		box-shadow:
			0 1px 0 rgba(255, 255, 255, 0.85) inset,
			0 1px 2px rgba(50, 30, 12, 0.18),
			0 2px 0 rgba(212, 197, 179, 0.95);
	}
	.radio-inputs .radio input:checked + .name {
		background: linear-gradient(180deg, #e9dfd2 0%, #f8f2e9 100%);
		font-weight: 600;
		color: #3a5b41;
		border-color: #cabaa6;
		box-shadow:
			inset 0 2px 5px rgba(65, 42, 20, 0.22),
